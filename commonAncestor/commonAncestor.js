...
Tree.prototype.getClosestCommonAncestor = function(child1, child2) {
  ...
  function searchAncestor(node) {
    //if both child are descendants of current node
    if (node.isDescendant(child1) && node.isDescendant(child2)) {
...
Tree.prototype.getAncestorPath = function(child) {
  //create array for path
  var ancestors = [this];
  //create a helper function
  function searchTree(node) {
  //check if current node has child as descendant
    if (node.isDescendant(child)) {
    //push current node to array
      ancestors.push(node);
    }
    //recurse function through children
    node.children.forEach(elem => {
      searchTree(elem);
    })
  }
  searchTree(this);
  //if this is not equal to child and child is not descendant of this
  if (this !== child && ancestors.length === 1) {
    return null;
  }
  return ancestors;
};
