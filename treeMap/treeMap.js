...
Tree.prototype.map = function (callback) {
  ...
  var mapTree = {value: callback(this.value), children: this.children};
    //if this.children is true
  function recurseTree(obj) {
    if (obj.children) {
      //for loop through each element in children
      for (var i = 0; i < obj.children.length; i++) {
        //run the callback on each of the objects with key "value" in children
        obj.children[i].value = callback(obj.children[i].value);
        recurseTree(obj.children[i]);
      }
    }
  }
  recurseTree(mapTree);
